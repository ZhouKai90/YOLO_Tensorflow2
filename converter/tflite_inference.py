...
# stride = [8,16,32]
stride = [16, 32, 64]
# outTensorName = ['Identity_1', 'Identity']
outTensorName = ['Identity_2', 'Identity_1', 'Identity']
...
if detection_type == 'helmet':
    ...
    parser.add_argument("--model", default='save/pedestrian/pedestrians_peleenet_yolov3_540_960_IO_uint8.tflite',
                        type=str, help="Model to load.")
    parser.add_argument("--anchors", default='data/anchors/pedestrian_480_640_9_anchors.txt', type=str,
    ...
    parser.add_argument("--model", default='save/head/head_peleenet_yolov3_540_960.tflite',
                        type=str, help="Model to load.")
...
def inference(interpreter, oriImgShape, imgData, anchors, stride, classNum):
    ...
    pred_bbox = []
    for n, tensor in enumerate(outTensorName):
        for i in range(len(output_details)):
            if output_details[i]['name'] == tensor:
                ...
                out = inference_decode(out, anchors, stride, n, classNum)
                pred_bbox.append(np.reshape(out, (-1, np.shape(out)[-1])))

    pred_bbox = np.concatenate(pred_bbox, axis=0)

    bboxes = postprocess_boxes(pred_bbox, oriImgShape, net_input_shape[1:3], args.score_threshold)
...
def image_inf(interpreter):
    ...
    for img in imgNameList:
        ...
        imgData = image_preprocess(imgData, input_shape[1:3], dtype='uint8')
...
def video_inf(interpreter, path):
    ...
    while True:
        ...
        bboxes = inference(interpreter, frame.shape[:2], imgData, anchors, stride, len(classes))
        if len(bboxes) > 0:
