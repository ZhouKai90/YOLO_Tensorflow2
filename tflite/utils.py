...
def get_anchors(anchors_path, branch):
    '''loads the anchors from a file'''
    with open(anchors_path) as f:
        anchors = f.readline()
    anchors = np.array(anchors.split(','), dtype=np.float32)
    return anchors.reshape(branch, 3, 2)
...
def postprocess_boxes(pred_bbox, org_img_shape, input_size, score_threshold):
    ...
    resize_ratio = min(input_size[2] / org_w, input_size[1] / org_h)

    dw = (input_size[2] - resize_ratio * org_w) / 2
...
def decode(conv_output, ANCHORS, STRIDES, i=0, classes_num=2):
    """
    return tensor of shape [batch_size, output_size, output_size, anchor_per_scale, 5 + num_classes]
            contains (x, y, w, h, score, probability)
    """
