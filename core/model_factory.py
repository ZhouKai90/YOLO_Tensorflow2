...
def get_model(train=True):
    ...
    elif CFG.TRAIN.BACKBONE == 'peleenet_yolov3':
        conv_tensors = peleenet_YOLOV3(input_tensor, NUM_CLASS)
    else:
...
def decode_backup(conv_output, i, ANCHORS, NUM_CLASS):
    """
...
def decode(conv_output, i, ANCHORS, NUM_CLASS):
    ...
    grid_y = tf.tile(tf.reshape(tf.range(0, grid_shape[0]), [-1, 1, 1, 1]),
                    [1, grid_shape[1], 1, 1])
    grid_x = K.tile(tf.reshape(tf.range(0, grid_shape[1]), [1, -1, 1, 1]),
                    [grid_shape[0], 1, 1, 1])
    xy_grid = tf.concat([grid_x, grid_y], axis=-1)
