...
class Dataset(object):
    ...
    def __init__(self, dataset_type, branch_size=3):
    ...
    def __next__(self):
        ...
        batch_label_sbbox = np.zeros((self.batch_size, self.train_output_sizes[0][0], self.train_output_sizes[0][1],
                                          self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
        batch_label_mbbox = np.zeros((self.batch_size, self.train_output_sizes[1][0], self.train_output_sizes[1][1],
                                      self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
        if self.branch_size == 3:
            batch_label_lbbox = np.zeros((self.batch_size, self.train_output_sizes[2][0], self.train_output_sizes[2][1],
                                          self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
            ...
            while num < self.batch_size:
                ...
                if self.branch_size == 3:
                    label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = \
                        self.preprocess_true_boxes(bboxes, self.branch_size)
                elif self.branch_size == 2:
                    label_sbbox, label_mbbox, sbboxes, mbboxes = self.preprocess_true_boxes(bboxes, self.branch_size)
    ...
    def parse_annotation(self, annotation):
        ...
        if not os.path.exists(image_name):
            raise KeyError("%s does not exist ... " % image_name)
        image = np.array(cv2.imread(image_name), dtype=np.uint8)
        if self.data_aug:
    ...
    def preprocess_true_boxes(self, bboxes, branch):

        assert len(self.train_output_sizes) == branch

        label = [np.zeros((self.train_output_sizes[i][0], self.train_output_sizes[i][1], self.anchor_per_scale,
                           5 + self.num_classes)) for i in range(branch)]
        bboxes_xywh = [np.zeros((self.max_bbox_per_scale, 4)) for _ in range(branch)]
        bbox_count = np.zeros((branch,))
