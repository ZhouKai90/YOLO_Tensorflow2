...
class Dataset(object):
    ...
    def __next__(self):
        ...
        batch_image = np.zeros((self.batch_size, self.train_input_size[0], self.train_input_size[1], 3),
                               dtype=np.float32)
        batch_label = [np.zeros((self.batch_size, self.train_output_sizes[i][0], self.train_output_sizes[i][1],
                                self.anchor_per_scale, 5 + self.num_classes), dtype=np.float32)
                                for i in range(self.branch_size)]
        batch_bboxes = [np.zeros((self.batch_size, self.max_bbox_per_scale, 4), dtype=np.float32)
        ...
        if self.batch_count < self.num_batchs:
            while num < self.batch_size:
                ...
                label, gt_bboxes = self.preprocess_true_boxes(bboxes, self.branch_size)
                assert len(label) == len(gt_bboxes) == self.branch_size

                batch_image[num, :, :, :] = image
                for i in range(self.branch_size):
                    batch_label[i][num, :, :, :, :] = label[i]
                    batch_bboxes[i][num, :, :] = gt_bboxes[i]
                num += 1
            self.batch_count += 1
            target = []
            for i in range(self.branch_size):
                branch_target = batch_label[i], batch_bboxes[i]
                target.append(branch_target)

            return batch_image, tuple(target)
